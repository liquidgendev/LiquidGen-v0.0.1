/*
  LiquidGenLanding.jsx
  Copied from root prototype and adapted for CRA dev
*/
  ...
  const requestFaucet = useCallback(async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setError(j.error || res.statusText || "Faucet request failed");
      }
    } catch (e) {
      console.error(e);
  ...
  return (
    <div className="mt-4">
      ...
      <div className="mt-2 flex gap-2">
        ...
        {txSig && (
          <a 
            className="px-3 py-2 border rounded hover:bg-sky-50 transition-colors" 
            target="_blank" 
            rel="noreferrer" 
      ...
      <div className="mt-2 space-y-1">
        <div className="text-sm text-slate-700">
          Balance: {balance === null ? 'loading...' : <span className="font-medium">{balance} $LQ</span>}
        </div>
        {error && (
          <div className="text-sm text-red-500">{error}</div>
        )}
        {txSig && !error && (
          <div className="text-sm text-emerald-600">Transaction successful!</div>
        )}
      </div>
    </div>
  );
...
function LandingInner({ connectionEndpoint }) {
  ...
  const connection = useMemo(() => new Connection(connectionEndpoint, "confirmed"), [connectionEndpoint]);
  const [walletError, setWalletError] = useState(null);

  useEffect(() => {
    if (walletError) {
      const timer = setTimeout(() => setWalletError(null), 5000);
      return () => clearTimeout(timer);
    }
  }, [walletError]);

  const [lockedValue, setLockedValue] = useState(10000000);
  const [apr, setApr] = useState(20);
  const [buybacksPerMonth, setBuybacksPerMonth] = useState(15);
  const [buybackAllocationPct, setBuybackAllocationPct] = useState(0.9);
  const [platformFeePct, setPlatformFeePct] = useState(0.02);
  const [lqBurnRatePct, setLqBurnRatePct] = useState(1);
      ...
      <header className="max-w-6xl mx-auto flex items-center justify-between py-8">
        ...
        <div className="flex gap-3 items-center">
          ...
          <div className="flex flex-col items-end">
            {connecting && <div className="text-sm text-amber-600">Connecting...</div>}
            {disconnecting && <div className="text-sm text-slate-600">Disconnecting...</div>}
            {connected && publicKey && (
              <>
                <div className="text-sm text-slate-600">
                  {wallet?.adapter?.name || 'Wallet'}: {publicKey.toBase58().slice(0, 6)}...{publicKey.toBase58().slice(-6)}
                </div>
                <div className="text-xs text-emerald-600">Connected to {network}</div>
              </>
            )}
            {walletError && (
